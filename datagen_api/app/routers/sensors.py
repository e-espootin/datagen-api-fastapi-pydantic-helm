...
@router.get("/sensors/temp", tags=["sensor_temp"], response_model=list[Temperature_Gen])
...
@router.get("/sensors/cobot", tags=["sensor_cobot"], response_model=list[Cobots_Gen])
async def read_sensor_Cobots(
    sensor_id: str = "cobot_1001", topic_name: str = "cobot_topic"
):
    try:
        return await Cobots(sensor_id=sensor_id, topic_name=topic_name).read_batch_data(num_samples=3)
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error getting senor temp: {str(e)}"
        )
